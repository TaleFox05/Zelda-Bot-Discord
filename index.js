...
client.on('messageCreate', async message => {
    ...
    if (command === 'crearcofre') {
        ...
        // --- NUEVA LÓGICA DE PARSING MUCHO MÁS ROBUSTA ---
        // Expresión regular para capturar el Canal ID (que va primero) y luego las dos cadenas entre comillas.
        ...
        // Esto ignora el Canal ID en la búsqueda de comillas
        const quotedRegex = /"([^"]+)"/g;
        ...
        const targetChannel = client.channels.cache.get(canalId);
        // Si !Zspawn funciona, esta línea DEBE funcionar
        if (!targetChannel) {
            return message.reply('¡Error crítico! El canal no se encontró. Si `!Zspawn` funciona, por favor, notifica al desarrollador.');
        }
