...
const CHEST_TYPES = {
    pequeño: {
        ...
        img: 'https://i.imgur.com/O6wo7s4.png' // Mantengo este si es funcional, si no lo puedes reemplazar
    },
    grande: {
...
function guardarCompendio() {
    try {
        ...
        console.error(`❌ [ERROR CRÍTICO DE PERSISTENCIA] ITEMS. Ruta: ${ITEMS_DATA_FILE}. Error: ${error.message}`);
    }
}
...
function cargarEnemigosBase() {
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            console.error(`[ERROR] Fallo al cargar enemies.json: ${error.message}`);
        }
    }
}
...
function guardarEnemigosBase() {
    try {
        ...
        console.log('✅ [PERSISTENCIA] Base de Enemigos guardada correctamente.');
    } catch (error) {
        console.error(`❌ [ERROR CRÍTICO DE PERSISTENCIA] ENEMIES. Ruta: ${ENEMIES_DATA_FILE}. Error: ${error.message}`);
    }
}
...
function createEnemyEmbedPage(enemies, pageIndex) {
    ...
    enemiesToShow.forEach(e => {
        // CORRECCIÓN: Se muestra solo el HP base (se eliminó la pluralización)
        embed.addFields({
            name: `**${e.nombre}**`,
            value: `**HP Base:** ${e.hp}`,
            inline: false
...
client.on('ready', () => {
    cargarCompendio();
    cargarEnemigosBase(); // Carga la base de enemigos
...
client.on('messageCreate', async message => {
    ...
    if (edicionActiva[userId] && edicionActiva[userId].channelId === message.channelId) {
        ...
        guardarCompendio(); // Guarda los cambios
        delete edicionActiva[userId];
